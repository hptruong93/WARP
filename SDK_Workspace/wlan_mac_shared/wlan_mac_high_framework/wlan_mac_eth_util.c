...
function_ptr_t transmit_callback;
u8 eth_encap_mode;
...
int wlan_eth_init() {
		...
		transmit_callback = (function_ptr_t)nullCallback;
...
void wlan_mac_util_set_transmit_callback(void(*callback)()){
	transmit_callback = (function_ptr_t)callback;
}

void set_eth_mac_addr(void* eth_mac_ptr) {
	memcpy((void*) &(eth_mac_addr[0]), eth_mac_ptr, 6);
}
...
#ifdef FMC_PKT_EN
	status = wlan_fmc_pkt_eth_send((u8*)eth_hdr, length);
