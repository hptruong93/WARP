	...
	switch(uart_mode){
		case UART_MODE_MAIN:
			switch(rxByte){
				...
				// Abdul's code
				case ASCII_4:
					//uart_mode = UART_SEND_ETHERNET_PACKET;
					curr_char = 0;
					xil_printf("\f");
					print_stats();
				break;
		...
		case UART_SEND_ETHERNET_PACKET:
			...
			switch(rxByte){
				...
				break;
				default:
					if( ((rxByte <= ASCII_z) && (rxByte >= ASCII_A)) || ((rxByte <= ASCII_9) && (rxByte >= ASCII_0) )){
					//the user entered a character
						//potential bug - 12358: might lead to memory corruption
						if(curr_char < 12358){
							xil_printf("%c", rxByte);
							text_entry[curr_char] = rxByte;
							curr_char++;							}
...
void print_menu(){
	...
	xil_printf("[4] - Print interface test statistics\n");
	xil_printf("[5] - Reset interface test statistics\n");
	xil_printf("\n");
